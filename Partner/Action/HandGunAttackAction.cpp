...
void HandGunAttackAction::Initialize()
{
	auto cowner = dynamic_cast<Character*>(m_common.m_owner);
	if (cowner) cowner->ChangeState(0, PlayerMotion::Shot, false);
...
bool HandGunAttackAction::Update(float deltaTime)
{
...
void HandGunAttackAction::Finalize()
{
	auto cowner = dynamic_cast<Character*>(m_common.m_owner);
	cowner->ChangeState(1, PlayerMotion::Idle, true);
}

bool HandGunAttackAction::Attack() const
{
	// <Partner*>でパートナーを取得
	// <Character*>でパートナーを取得
