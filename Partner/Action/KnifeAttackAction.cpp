...
constexpr float attackPower = 3.0f;

void KnifeAttackAction::Initialize()
{
	auto cowner = dynamic_cast<Character*>(m_common.m_owner);
	if (cowner) cowner->ChangeState(0, PlayerMotion::Attack, false);
...
bool KnifeAttackAction::Update(float deltaTime)
{
...
void KnifeAttackAction::Finalize()
{
	auto cowner = dynamic_cast<Character*>(m_common.m_owner);
	cowner->ChangeState(1, PlayerMotion::Idle, true);
}

bool KnifeAttackAction::Attack() const
{
	// <Partner*>でパートナーを取得
	// <Character*>でパートナーを取得
