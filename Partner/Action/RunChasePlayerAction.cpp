#include "RunChasePlayerAction.h"
...
void RunChasePlayerAction::Initialize()
{
	...
	if (cowner) cowner->ChangeState(0, PlayerMotion::Run, true);
	auto powner = dynamic_cast<Partner*>(m_common.m_owner);
	powner->ChangeWeapon(Weapon::WeaponType::Knife);
}

bool RunChasePlayerAction::Update(float deltaTime)
{
	// プレイヤーを取得
...
void RunChasePlayerAction::Finalize()
{

}

bool RunChasePlayerAction::isClose() const
{
	// プレイヤーを取得
	auto player = m_common.m_world->FindActor("Player");
	if (!player) return false;
